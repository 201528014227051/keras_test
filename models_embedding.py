...
from keras.layers import BatchNormalization, Embedding, Activation, Reshape, Flatten
...
def NIC(max_token_length, vocabulary_size, embedding_matrix, rnn='lstm', num_image_features=2048,
 hidden_size=512, embedding_size=512, regularizer=1e-8):
    ...
    return model
'''
    output1 = TimeDistributed(Dense(units=1,
                                    kernel_regularizer=l2(regularizer),
                                    activation='softmax'))(recurrent_network)
    output = Flatten(name = 'output')(output1)
'''
